...
static void dump_array1000(const int x[])
{
#ifdef DEBUG
	const int n = 1000;
	int i, width = 20;
...
static void generate_array1000(int x[], enum dup_pos p, int count)
{
	const int n = 1000;
	int i, value, start;
...
int test_find_first1000(int *i)
{
	...
	for (t = tests; t->name; t++) {
		...
		printf("%3d) %-25s: %-45s", ++(*i), "test_find_first1000",
		       t->name);

		generate_array1000(x, t->dup_pos, t->dup_count);
		got = find_first1000(t->v, x);
		if (got != t->want) {
